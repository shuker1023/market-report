...
def generate_analysis(fund_data: str) -> str:
    """生成分析报告"""
    prompt = f"""你是一位拥有15年以上经验的公募基金专业交易员，风格稳健偏价值，擅长结合宏观、行业、估值和资金面分析。

基于以下基金持仓数据，写一篇专业分析报告和60秒口播稿。
...
要求：
1. **分析报告**（约500字）：
   - 从专业交易员视角，综合分析用户持仓两只科技主题基金的表现
   - 结合重仓股（光通信、PCB、半导体等）当日行情解读净值波动原因
   - 对比两只基金的特点差异（永赢科技 vs 易方达信息产业）
   - 分析当前科技/AI板块整体趋势
   - 给出持有建议（注意：用"分析参考"语气，避免绝对化预测，强调风险）
2. **60秒口播稿**（约250字）：口语化、适合朗读，开头用"各位投资者朋友大家好"
3. 在文章末尾给出**后市关注要点**
4. 格式：先用"【文章】"标记，再用"【口播】"标记
